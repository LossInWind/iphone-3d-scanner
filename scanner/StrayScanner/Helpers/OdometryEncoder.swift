...
class OdometryEncoder {
    ...
    func add(frame: ARFrame, currentFrame: Int) {
        addPose(transform: frame.camera.transform, timestamp: frame.timestamp, frameNumber: currentFrame)
    }
    
    func addPose(transform: simd_float4x4, timestamp: TimeInterval, frameNumber: Int) {
        transforms.append(transform)
        let xyz: vector_float3 = getTranslation(T: transform)
        let q_WA = simd_quatf(transform)
        let q: vector_float4 = (q_WA * q_AC).vector
        let frameNumberStr = String(format: "%06d", frameNumber)
        let line = "\(timestamp), \(frameNumberStr), \(xyz.x), \(xyz.y), \(xyz.z), \(q.x), \(q.y), \(q.z), \(q.w)\n"
